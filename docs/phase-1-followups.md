...
| 13 | ~~No runtime validation of API responses~~ | **Partially fixed.** Boundary parsers in `src/api/parse.ts` (primitives + `SchemaError`), `parse-me.ts` (highest-value endpoint), and `parse-shelf.ts` (list, item, ok) validate the two most-consumed shapes and throw a `SchemaError` that names the exact field path (`user.email`, `entries[3].day`, `items[1].kind`). `failures.ts` classifies it as a `'schema'` kind with copy that never claims data loss. Types moved into `types-me.ts`/`types-shelf.ts` so parsers stay react-free and testable in plain Node. 20-test contract in `test/parse.test.js`. **Remaining:** parsers for auth, scan, entries, notifications, safety, password endpoints. |
